      ...
      <div className="absolute inset-x-0 bottom-0 p-8 md:p-10 space-y-6 z-10">
        <div className="space-y-3">
            <div className="flex items-center gap-2">
                ...
                {channel.totalViews > 1000 && (
                    <Badge variant="secondary" className="bg-white/10 text-white border-none backdrop-blur-md rounded-full px-4 py-1 font-black uppercase tracking-widest text-[8px]">
                        Trending
                    </Badge>
                )}
            </div>
        ...
        <div className="flex items-center gap-4">
            <Avatar className="h-10 w-10 border-2 border-white/20">
                ...
                <AvatarFallback className="bg-primary/20 text-white font-black text-xs">{channel.teacher.name.substring(0,2).toUpperCase()}</AvatarFallback>
            </Avatar>
            <div className="text-start">
                <p className="text-sm font-black text-white">{channel.teacher.name}</p>
                <p className="text-[9px] font-black uppercase tracking-widest text-white/40">{channel.totalViews.toLocaleString()} Views</p>
            </div>
        </div>
    ...
    <div className="container-center section-wrapper !pt-10">
      ...
      <header className="space-y-12 mb-20 relative z-10">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
            <div className="space-y-4 text-start">
                ...
                <p className="text-lg md:text-xl text-muted-foreground font-medium max-w-xl">
                    Discover the best educators in Egypt through high-definition channels and immersive trailers.
                </p>
            </div>
        ...
        <div className="flex items-center gap-8 overflow-x-auto pb-4 scrollbar-hide">
            {["All", "Physics", "Mathematics", "Biology", "Chemistry", "Languages"].map((cat) => (
                <button key={cat} className="shrink-0 px-8 py-3 rounded-full border border-border/40 bg-card hover:bg-primary hover:text-white hover:border-primary transition-all font-black uppercase tracking-widest text-[10px]">
                    {cat}
                </button>
            ))}
        </div>
      ...
      <div className="relative z-10">
        {isLoading ? (
            <div className="flex flex-col items-center justify-center h-[400px] gap-6">
                ...
                <p className="text-xs font-black uppercase tracking-[0.3em] text-muted-foreground/40">Loading Broadcasts...</p>
            </div>
        ) : (
      ...
      {!isLoading && channels?.data.length === 0 && (
          <div className="text-center py-40 space-y-6">
              ...
              <p className="text-muted-foreground font-medium">Try searching for a different subject or teacher name.</p>
          </div>
      )}
    </div>
  );
