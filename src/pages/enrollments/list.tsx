...
const EnrollmentsList = () => {
  ...
  usePageTitle(t("my-classes.title"));
  ...
  const {
    query: { data: enrollmentsData, isLoading },
  } = useList<Enrollment>({
    resource: "my-classes",
  ...
  const handleStatusUpdate = (id: number, status: "approved" | "rejected") => {
    updateStatus(
      ...
      {
        onSuccess: () => {
          toast.success(t("my-classes.toasts.statusUpdate", { status }));
          invalidate({ resource: "my-classes", invalidates: ["list"] });
        },
      }
    );
  };
  ...
  const handleBulkAction = (status: "approved" | "rejected") => {
    ...
    toast.promise(Promise.all(promises), {
      loading: t("my-classes.toasts.processing", {
        count: selectedIds.length,
      }),
      success: () => {
        setSelectedIds([]);
        invalidate({ resource: "my-classes", invalidates: ["list"] });
        return t("my-classes.toasts.bulkSuccess", {
          count: selectedIds.length,
        });
      },
      error: t("my-classes.toasts.bulkError"),
    });
  };
        ...
        <div className="space-y-10">
          ...
          >
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div>
                <h1 className="text-4xl font-black tracking-tight">{t("my-classes.title")}</h1>
                <p className="text-muted-foreground font-medium mt-1">
                  {t("my-classes.description")}
                </p>
              </div>
          ...
          <Card className="p-4 border-primary/5 bg-muted/30 rounded-4xl backdrop-blur-sm">
            <div className="flex flex-col lg:flex-row gap-4">
              <div className="relative flex-1 group">
                ...
                <Input
                  type="text"
                  placeholder={t("my-classes.searchPlaceholder")}
                ...
                {selectedIds.length > 0 && (
                  <div className="flex items-center gap-2 bg-primary/5 px-4 rounded-2xl border border-primary/10 animate-in fade-in slide-in-from-end-4">
                    <span className="text-[10px] font-black uppercase tracking-widest text-primary">
                      {selectedIds.length} {t("common.cases")}
                    </span>
                    <Button
                      size="sm"
                      variant="ghost"
                      className="h-8 text-green-600 hover:bg-green-50 font-black text-[10px] uppercase tracking-widest"
                    ...
                    <Button
                      size="sm"
                      variant="ghost"
                      className="h-8 text-destructive hover:bg-destructive/5 font-black text-[10px] uppercase tracking-widest"
          ...
          >
            {isLoading ? (
              ...
              <div className="h-full w-full flex items-center justify-center p-12">
                <EmptyState
                  icon={Users}
                  title={t("my-classes.empty.title")}
                  description={t("my-classes.empty.desc")}
                          ...
                          <div className="flex flex-wrap items-center justify-center md:justify-start gap-x-6 gap-y-2 mt-3">
                            ...
                            <div className="flex items-center gap-2 text-muted-foreground">
                              ...
                              <span className="text-xs font-bold uppercase tracking-tight">
                                {t("my-classes.requested", {
                                  time: enrollmentDate.fromNow(),
                                })}
                              </span>
                            </div>
                          </div>
                          ...
                          <DropdownMenu>
                            ...
                            >
                              ...
                              >
                                ...
                                <span className="font-bold">{t("my-classes.viewProfile")}</span>
                              </DropdownMenuItem>
                              <DropdownMenuItem
                              ...
                              >
                                ...
                                <span className="font-bold">{t("my-classes.viewClass")}</span>
                              </DropdownMenuItem>
                              ...
                              >
                                ...
                                <span className="font-bold">{t("my-classes.remove")}</span>
                              </DropdownMenuItem>
                            </DropdownMenuContent>
                          </DropdownMenu>
                        </div>
          ...
          <AlertDialogFooter className="gap-3 pt-6">
            ...
            <AlertDialogAction
              onClick={() => {
                if (deleteTarget) {
                  unenroll(
                    { resource: "my-classes", id: deleteTarget },
                    {
                      onSuccess: () => {
                        toast.success(t("my-classes.toasts.removed"));
