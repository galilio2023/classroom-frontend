          ...
          >
            ...
            <CardHeader className="p-8 pb-4 relative z-10">
              <div className="flex items-center justify-between">
                ...
                <Badge
                  variant="secondary"
                  className="rounded-full px-3 py-1 font-black text-[9px] uppercase tracking-widest bg-ai-primary/10 text-ai-primary border-none shadow-sm"
                >
            ...
            <CardContent className="p-8 pt-2 space-y-8 relative z-10">
              <p className="text-sm text-muted-foreground font-medium leading-relaxed">
                {t("dashboard.staff.promos.aiAssistant.description")
                  .split("Gemini 3 Flash Preview")
                  .map((part, i, arr) => (
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  {
                    icon: Wand2,
                    label: t("dashboard.staff.promos.aiAssistant.quizGen"),
                  },
                  {
                    icon: MessageSquare,
                    label: t("dashboard.staff.promos.aiAssistant.aiChat"),
                  },
                ].map((feat) => (
                  <div
                    key={feat.label}
                    className="flex items-center gap-2.5 p-3 rounded-xl bg-background/50 dark:bg-muted/30 border border-border/40 shadow-sm transition-colors group-hover:bg-background/80 dark:group-hover:bg-muted/50"
                  >
        ...
        >
          <Card className="border-none shadow-2xl bg-card/50 backdrop-blur-xl rounded-4xl overflow-hidden group text-start">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">
                    {t("dashboard.staff.promos.studentSupport.badge")}
                  </p>
                </div>
              </div>
            </CardHeader>
            <CardContent className="p-8 pt-2 space-y-6">
              <p className="text-sm text-muted-foreground font-medium leading-relaxed">
                {t("dashboard.staff.promos.studentSupport.description")
                  .split("AI Study Buddy")
                  .map((part, i, arr) => (
                    <React.Fragment key={i}>
                      {part}
                      {i < arr.length - 1 && (
