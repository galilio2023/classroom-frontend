        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <Dialog open={isLinkDialogOpen} onOpenChange={setIsLinkDialogOpen}>
            <DialogTrigger asChild>
              <Button size="lg" className="flex-1 md:flex-none rounded-2xl h-12 md:h-14 px-8 font-black uppercase tracking-widest text-[10px] gap-2 shadow-xl shadow-primary/20 transition-all hover:scale-105 active:scale-95">
            ...
            <DialogContent className="rounded-[2.5rem] border-none shadow-2xl bg-card/95 backdrop-blur-xl max-w-lg p-0 overflow-hidden text-start">
              <div className="p-8 md:p-12 space-y-8">
                ...
                <div className="space-y-6">
                    ...
                    <Label htmlFor="email" className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground ms-2">
                        {t("dashboard.parent.studentEmail")}
                    </Label>
                    <div className="relative group">
                        <Search className={cn("absolute top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground/40 group-focus-within:text-primary transition-colors", "start-6")} />
                        <Input
                            id="email"
                            placeholder="student@example.com"
                            value={studentEmail}
                            onChange={(e) => setStudentEmail(e.target.value)}
                            className={cn("h-16 rounded-3xl bg-muted/30 border-none shadow-inner text-lg font-black placeholder:text-muted-foreground/30 focus-visible:ring-primary/20", "ps-14 pe-8")}
                        />
                    </div>
                    </div>
                </div>
            ...
            <Card className="border-2 border-dashed border-border/40 bg-card/20 rounded-[2.5rem] md:rounded-[3.5rem] overflow-hidden">
                <CardContent className="flex flex-col items-center justify-center py-20 md:py-32 text-center space-y-6">
                    ...
                    <div className="space-y-2">
                        <h3 className="text-2xl md:text-3xl font-black tracking-tight">{t("dashboard.parent.noChildren")}</h3>
