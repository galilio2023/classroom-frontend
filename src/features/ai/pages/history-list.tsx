...
const AIHistoryList = () => {
  ...
  const getToolIcon = (tool: string) => {
    switch (tool) {
      ...
      default:
        return { icon: History, color: "text-primary", bg: "bg-primary/10" };
    }
  };

  return (
    <div
      className="space-y-8 md:space-y-12 pb-20 max-w-7xl mx-auto px-4"
      dir={isAr ? "rtl" : "ltr"}
    >
      {/* Header */}
      <motion.div
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <p className="text-muted-foreground font-medium animate-pulse">
            {t("aiHub.history.loading", {
              defaultValue: "Loading your history...",
            })}
          </p>
        </div>
      ) : data?.length === 0 ? (
        <Card className="border-dashed border-2 border-border/40 bg-muted/10 rounded-[2.5rem] py-20 flex flex-col items-center justify-center text-center px-6">
          ...
          <h3 className="text-2xl font-black tracking-tight mb-2">
            {t("aiHub.history.empty.title", {
              defaultValue: "No history found",
            })}
          </h3>
          <p className="text-muted-foreground max-w-sm mb-8">
            {t("aiHub.history.empty.description", {
              defaultValue:
          ...
          >
            <a href="/ai-study-lab">
              {t("aiHub.history.empty.cta", {
                defaultValue: "Go to Study Lab",
              })}
            </a>
          </Button>
        </Card>
                ...
                >
                  ...
                  <CardContent className="p-6 pt-0 space-y-6">
                    <div className="h-12 line-clamp-2 text-sm text-muted-foreground/80 leading-relaxed font-medium">
                      {log.action === "flashcards"
                        ? t("aiHub.studyLab.tools.flashcards.generated", {
                            defaultValue: "Flashcard Deck generated",
                          })
                        : log.response}
                    </div>
