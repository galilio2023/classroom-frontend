...
import { FlashcardPlayer } from "@/features/quizzes/components/flashcard-player";
...
const AIStudyLab = () => {
  ...
  const toolCategories = [
    {
      name: t("aiHub.studyLab.categories.creation"),
      tools: ["quiz", "flashcards"],
    },
    {
      name: t("aiHub.studyLab.categories.analysis"),
      tools: ["explain", "summary"],
    },
  ];
          ...
          <div className="space-y-8">
            {toolCategories.map((category) => (
              <div key={category.name} className="space-y-4">
                <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground/60 px-2">
                  {category.name}
                </h3>
                  ...
                  {category.tools.map((toolId) => {
                    ...
                    return (
                      ...
                      >
                        <Card
                          className={cn(
                            "h-full cursor-pointer transition-all duration-500 border border-border/40 shadow-sm rounded-3xl overflow-hidden group relative",
                            activeTool === tool.id
