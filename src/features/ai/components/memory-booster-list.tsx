...
export const MemoryBoosterList: React.FC<MemoryBoosterListProps> = ({ onSelectTopic }) => {
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-top-4 duration-700">
      <div className="flex items-center justify-between px-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-black tracking-tight">{t("aiHub.studyLab.memoryBooster.title", "Daily Memory Missions")}</h2>
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
                {t("aiHub.studyLab.memoryBooster.subtitle", "Based on Spaced Repetition (SM-2)")}
            </p>
          </div>
        </div>
        {dueItems.length > 0 && (
            <div className="px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest border bg-orange-500 text-white border-none animate-pulse">
                {dueItems.length} {t("aiHub.studyLab.memoryBooster.dueToday", "Due Today")}
            </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {dueItems.map((item: SpacedRepetition, idx: number) => (
          ...
          >
            <Card className="ai-card-premium group relative overflow-hidden cursor-pointer border-2 border-transparent hover:border-ai-primary/30 transition-all duration-500 animate-ai-pulse" onClick={() => onSelectTopic(item.topic)}>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="space-y-3 flex-1 text-left rtl:text-right">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-black text-orange-600 uppercase tracking-[0.2em]">Priority Review</span>
                    </div>
                    <h3 className="text-xl font-black leading-tight group-hover:text-ai-primary transition-colors tracking-tight">{item.topic}</h3>
        ...
        {upcomingItems.slice(0, 2).map((item: SpacedRepetition) => (
          <Card key={item.id} className="bg-muted/30 border-border/40 opacity-60">
            <CardContent className="p-6 flex items-center justify-between">
              <div className="space-y-1">
                <h3 className="text-sm font-bold truncate max-w-[150px]">{item.topic}</h3>
