...
const ForgotPasswordPage = () => {
  ...
  const forgotPasswordSchema = z.object({
    identifier: z
      .string()
      .min(1, t("auth.login.identifierRequired", "Email or phone number is required")),
  });
  ...
  const onSubmit = async (values: z.infer<typeof forgotPasswordSchema>) => {
    setIsPending(true);
    try {
      await axios.post(`${BASE_URL}/identity/forgot-password`, values);
      toast.success(t("auth.forgotPassword.codeSent", "Recovery code sent via WhatsApp!"));
      navigate(`/reset-password?identifier=${encodeURIComponent(values.identifier)}`);
    } catch (err: any) {
      const errorMessage =
            ...
            <div className="space-y-2">
              ...
              <CardDescription className="font-medium text-base md:text-lg px-4 md:px-8">
                {t(
                  ...
                  "No worries! Enter your email or phone number and we'll send you a recovery code via WhatsApp."
                )}
              </CardDescription>
            </div>
          </CardHeader>
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 md:space-y-8">
                  ...
                  render={({ field }) => (
                    <FormItem className="space-y-3">
                      <FormLabel className="font-black uppercase text-[10px] tracking-[0.2em] text-muted-foreground/60 ms-2">
                        {t("auth.forgotPassword.identifierLabel", "Email or Phone Number")}
                      </FormLabel>
                      <FormControl>
                        <Input
                          placeholder={t(
                            "auth.forgotPassword.identifierPlaceholder",
                            "e.g. user@example.com or +201234567890"
                          )}
                ...
                >
                  {isPending ? (
                    ...
                    <div className="flex items-center gap-3">
                      {t("auth.forgotPassword.sendCode", "Send Recovery Code")}
                      <Zap
                        className={cn(
