...
export const SidebarBadge = ({ resource, className }: SidebarBadgeProps) => {
  ...
  return (
    <div
      className={cn(
        "absolute -top-1 -end-1 flex h-4 w-4 items-center justify-center rounded-full bg-primary text-[10px] font-black text-primary-foreground shadow-sm animate-in zoom-in duration-300",
        className
      )}
    >
      {count > 9 ? "9+" : count}
