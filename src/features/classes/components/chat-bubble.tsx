...
export const ChatBubble = ({ post, isOwn, onDelete, onReply, onSolve }: ChatBubbleProps) => {
  ...
  useEffect(() => {
    // 🛡️ PERFORMANCE: Only overwrite if the incoming replies have more data than we currently have
    // OR if the first few items have changed (indicating a reorder or deletion).
    if (post.replies && post.replies.length > 0) {
      const hasNewData =
        post.replies.length > fullReplies.length || post.replies[0]?.id !== fullReplies[0]?.id;
          ...
          >
            ...
            {isStaff && post.parentId && post.user?.id !== post.solvedBy?.id && (
              <Button
                ...
                title={t("common.markAsSolved", "Mark as Solved")}
                aria-label={t("common.markAsSolved", "Mark as Solved")}
              >
