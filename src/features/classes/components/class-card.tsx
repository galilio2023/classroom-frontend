...
export const ClassCard = React.memo(
  ({
    classItem,
    index,
    identity,
        ...
        <div className="flex items-center gap-3 pt-4 border-t border-border/40 w-full mt-auto">
          {isTeacher && !isAssigned ? (
            <Button
              onClick={(e) => {
                e.stopPropagation();
                onApply(classItem.id as number, classItem.name);
              }}
              disabled={!!pendingApp}
              size="lg"
            ...
            <Button
              onClick={(e) => {
                e.stopPropagation();
                onEnroll(classItem.id as number);
              }}
              ...
              {((isTeacher && isAssigned) || isAdmin) && (
                <>
                  ...
                  <DropdownMenuItem
                    onClick={(e) => {
                      e.stopPropagation();
                      onClone(classItem.id as number);
                    }}
                    className="rounded-xl gap-3 py-3 cursor-pointer"
                  >
                    <div className="p-2 rounded-lg bg-primary/10 text-primary">
                  ...
                  <DropdownMenuItem
                    onClick={(e) => {
                      e.stopPropagation();
                      onDelete(classItem.id as number);
                    }}
  ...
  (prev, next) => {
    // 🚀 PERFORMANCE: Deep compare essential props
    return (
      prev.classItem.id === next.classItem.id &&
      prev.classItem.isLive === next.classItem.isLive &&
      prev.classItem.version === next.classItem.version &&
      prev.classItem._count?.enrollments === next.classItem._count?.enrollments &&
