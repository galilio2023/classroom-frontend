      ...
      <Card className="overflow-hidden border-border/40 shadow-2xl rounded-[2.5rem] md:rounded-[3rem] bg-card/50 backdrop-blur-3xl">
        ...
        <CardContent className="relative pt-0 flex flex-col items-center text-center px-6 md:px-8 pb-10 md:pb-12 text-start">
          <Avatar className="h-32 w-32 md:h-40 md:w-40 border-8 border-background -mt-16 shadow-2xl rounded-4xl md:rounded-[2.5rem]">
            ...
            <AvatarFallback className="text-4xl md:text-5xl font-black bg-primary text-primary-foreground">{user.name[0]}</AvatarFallback>
          </Avatar>
          
          <div className="mt-6 space-y-2">
            <h2 className="text-3xl md:text-4xl font-black tracking-tight text-balance">{user.name}</h2>
            <div className="flex items-center justify-center gap-2">
                <Badge variant="secondary" className="capitalize font-black px-4 py-1.5 rounded-full text-[10px] tracking-widest shadow-sm">{t(`roles.${user.role.toLowerCase()}` as any)}</Badge>
            ...
            <div className="w-full mt-8 space-y-6 bg-primary/5 p-6 rounded-4xl border border-primary/5 shadow-inner">
              <div className="flex items-center justify-between px-2">
                <div className="flex flex-col items-start">
                  <span className="text-3xl md:text-4xl font-black text-primary">{formatter.format(currentLevel)}</span>
                ...
                <div className="flex flex-col items-end">
                  <span className="text-3xl md:text-4xl font-black text-primary">{formatter.format(user.xp || 0)}</span>
          ...
          {isStudent && (
            <div className="w-full mt-4 grid grid-cols-2 gap-4">
              <div className="bg-orange-500/10 p-4 md:p-6 rounded-2xl border border-orange-500/10 flex flex-col items-center shadow-sm text-start">
                ...
                <span className="text-xl md:text-2xl font-black text-orange-600">{formatter.format(user.currentStreak || 0)}</span>
              ...
              <div className="bg-primary/10 p-4 md:p-6 rounded-2xl border border-primary/10 flex flex-col items-center shadow-sm text-start">
                ...
                <span className="text-xl md:text-2xl font-black text-primary">{formatter.format(user.longestStreak || 0)}</span>
      ...
      {isTeacher && user.teacherChannel && (
        <Card className="border-border/40 shadow-xl rounded-[2.5rem] md:rounded-[3rem] bg-card/50 backdrop-blur-3xl overflow-hidden p-6 md:p-8 text-start">
            <div className="flex items-center gap-3 mb-4">
                <div className="p-2.5 rounded-xl bg-primary/10 text-primary shadow-sm border border-primary/5"><Tv className="h-5 w-5" /></div>
                <h4 className="text-xl md:text-2xl font-black tracking-tight">{t("teacherChannel.labels.stats" as any)}</h4>
            </div>
            <div className="space-y-4">
                <div className="flex justify-between items-center"><span className="text-sm md:text-base font-bold text-muted-foreground">{t("teacherChannel.labels.views" as any)}</span><span className="text-lg md:text-xl font-black">{user.teacherChannel.totalViews.toLocaleString()}</span></div>
                <div className="flex justify-between items-center"><span className="text-sm md:text-base font-bold text-muted-foreground">{t("teacherChannel.labels.conversion" as any)}</span><span className="text-lg md:text-xl font-black text-primary">{(user.teacherChannel.conversionRate * 100).toFixed(1)}%</span></div>
            </div>
        </Card>
      )}
    </div>
