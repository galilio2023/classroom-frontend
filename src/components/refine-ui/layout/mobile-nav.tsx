...
export const MobileNav = () => {
  ...
  return (
    <motion.div 
      ...
      className="md:hidden fixed bottom-0 left-0 right-0 z-[60] bg-background/80 backdrop-blur-xl border-t border-border/40 px-6 py-2 pb-safe-area flex justify-between items-center shadow-[0_-8px_32px_rgba(0,0,0,0.1)] rounded-t-[1.5rem]"
    >
      {navItems.map((item) => {
        ...
        return (
          ...
          >
            <motion.div
              ...
              className="relative"
            >
              <item.icon className={cn("h-5 w-5 md:h-6 md:w-6 transition-transform", isActive && "scale-110")} />
              {isActive && (
                <motion.div
