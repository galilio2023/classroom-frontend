  ...
  return (
    <section id="features" className="container mx-auto px-4 py-20 md:py-32 text-start">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 md:gap-12 mb-16 md:mb-24">
          ...
          <Button variant="ghost" className="font-black uppercase tracking-widest text-[10px] md:text-xs gap-3 h-14 md:h-16 px-8 md:px-10 rounded-2xl hover:bg-primary/5 group border border-primary/10 transition-all w-fit">
        ...
        <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-50px" }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {features.map((feature, index) => (
            <motion.div key={index} variants={item} whileHover={{ y: -8, scale: 1.01 }} className="p-8 md:p-12 bg-card/40 hover:bg-card border-2 border-transparent hover:border-primary/20 rounded-[2.5rem] md:rounded-[3.5rem] shadow-xl transition-all group relative overflow-hidden">
              ...
              <h3 className="text-xl md:text-2xl font-black mb-3 md:mb-4 uppercase tracking-tight">{feature.title}</h3>
              <p className="text-sm md:text-lg text-muted-foreground/80 font-medium leading-relaxed">{feature.description}</p>
            </motion.div>
          ))}
        </motion.div>
    </section>
