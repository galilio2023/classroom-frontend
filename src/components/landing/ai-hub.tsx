        ...
        <div className="container mx-auto relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 md:gap-24 items-center">
            ...
            >
              ...
              <Button size="lg" className="rounded-2xl h-14 md:h-16 px-8 md:px-10 font-black uppercase tracking-widest text-[10px] md:text-xs shadow-xl shadow-primary/20 hover:scale-105 transition-all w-full sm:w-auto" asChild>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="relative">
              ...
              <div className="relative bg-card border-2 md:border-4 border-primary/10 rounded-[2.5rem] md:rounded-[4rem] p-6 md:p-10 shadow-2xl backdrop-blur-xl">
                ...
                <div className="space-y-5 md:space-y-6 text-start">
                  <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} className="bg-muted/40 p-5 md:p-6 rounded-[1.5rem] md:rounded-3xl rounded-ss-none max-w-[90%] border border-border/40 shadow-sm">
                    <p className="text-sm md:text-base font-bold leading-relaxed italic opacity-80">{t("landing.ai.chat.msg1")}</p>
                  </motion.div>
                  <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }} className="bg-primary p-5 md:p-6 rounded-[1.5rem] md:rounded-3xl rounded-ee-none max-w-[90%] ms-auto text-primary-foreground shadow-xl shadow-primary/20">
                    <p className="text-sm md:text-base font-bold leading-relaxed">{t("landing.ai.chat.msg2")}</p>
                  </motion.div>
                  <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: 1 }} className="bg-muted/40 p-5 md:p-6 rounded-[1.5rem] md:rounded-3xl rounded-ss-none max-w-[95%] border border-border/40 shadow-sm">
                    ...
                    <div className="mt-6 space-y-3">
                      ...
                      <div className="h-1.5 w-full bg-primary/10 rounded-full overflow-hidden shadow-inner">
                        <motion.div initial={{ width: 0 }} whileInView={{ width: "84%" }} transition={{ duration: 2, delay: 1.5 }} className="h-full bg-primary rounded-full shadow-[0_0_10px_rgba(var(--primary),0.5)]" />
                      </div>
                    </div>
                  </motion.div>
                </div>
