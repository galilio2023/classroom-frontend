  ...
  return (
    <Card className="border shadow-md bg-card/50 backdrop-blur-xl">
      <CardHeader className="pb-2">
        ...
        <CardDescription>Your assignment submission frequency over the last 6 months.</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="flex flex-col gap-1 overflow-x-auto pb-2">
          <div className="flex gap-1">
            <div className="grid grid-rows-7 gap-1 pr-2">
              {days.map(day => (
                <span key={day} className="text-[10px] text-muted-foreground h-3 flex items-center">{day}</span>
              ))}
            </div>
              ...
              <TooltipProvider>
                {heatmapData.map((day, i) => (
                  <Tooltip key={i}>
                    ...
                    <TooltipContent>
                      <p className="text-xs">
                        <span className="font-bold">{day.count} submissions</span> on {day.date.toLocaleDateString()}
                      </p>
                    </TooltipContent>
                  </Tooltip>
                ))}
