...
import { Clock, User, ArrowRight, FileText, Sparkles, AlertCircle } from "lucide-react";
        ...
        <div className="flex items-center p-5 gap-5">
          <div className="relative">
            ...
            <Avatar className="h-14 w-14 border-2 border-background shadow-lg relative z-10 transition-transform duration-500 group-hover:scale-110">
              ...
              <AvatarFallback className="bg-primary/5 text-primary font-black">
                {submission.student?.name?.[0] || <User className="h-6 w-6" />}
              </AvatarFallback>
            </Avatar>
            {submission.isLate && (
          ...
          <div className="flex-1 min-w-0 space-y-1">
            <div className="flex items-center gap-3">
              <p className="text-base font-black tracking-tight text-foreground group-hover:text-primary transition-colors truncate">
            ...
            <div className="flex items-center gap-2 mt-1.5">
              <span className="text-[9px] font-black text-muted-foreground/40 uppercase tracking-[0.2em] flex items-center gap-1.5">
                <Clock className="h-2.5 w-2.5" />
                Submitted {formatDistanceToNow(new Date(submission.createdAt), { addSuffix: true })}
              </span>
              <div className="w-1 h-1 rounded-full bg-muted-foreground/20" />
              <div className="flex items-center gap-1 text-[9px] font-black uppercase tracking-widest text-ai-primary/60">
