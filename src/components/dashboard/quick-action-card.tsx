...
export const QuickActionCard: React.FC<QuickActionCardProps> = ({ action, onAction }) => {
  ...
  return (
    ...
    >
      <Card 
        className="h-full border-none shadow-sm hover:shadow-xl group transition-all duration-500 bg-card/40 backdrop-blur-3xl rounded-[1.5rem] md:rounded-[2.5rem] overflow-hidden relative cursor-pointer"
        onClick={() => action.resource && onAction(action.resource)}
      >
        {/* Modern Background Icon Glow */}
        <div className="absolute -right-6 -top-6 p-4 opacity-[0.02] group-hover:opacity-[0.06] transition-all duration-700 rotate-12 group-hover:rotate-0 group-hover:scale-125 group-hover:text-primary pointer-events-none">
        ...
        <CardContent className="p-5 md:p-8 pt-1 md:pt-2 relative z-10 flex flex-col h-full">
          ...
          <div className="flex items-center justify-between mt-auto pt-4 md:pt-6 border-t border-border/40">
            ...
            <Button 
              variant="ghost"
              size="sm"
              className="rounded-xl bg-primary/5 hover:bg-primary hover:text-primary-foreground transition-all font-black uppercase tracking-widest text-[10px] h-10 md:h-12 px-5 md:px-8 gap-2 shadow-sm group-hover:shadow-lg group-hover:shadow-primary/20 ml-auto"
            >
              Explore
