    ...
    return (
        <div className="space-y-8">
            {isStaff && (
                ...
                >
                  <Card 
                    className="relative group cursor-pointer overflow-hidden border-none shadow-2xl bg-ai-primary/[0.03] backdrop-blur-xl rounded-[2rem] border border-ai-primary/10 text-left rtl:text-right" 
                  ...
                  >
                      ...
                      <div className="absolute ltr:-right-4 rtl:-left-4 -top-4 p-4 opacity-[0.05] group-hover:opacity-[0.1] transition-all duration-700 ltr:rotate-12 rtl:-rotate-12 group-hover:rotate-0 group-hover:scale-125">
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            {[
                              ...
                              <div key={feat.label} className="flex items-center gap-2 p-2 rounded-xl bg-white/50 dark:bg-zinc-900/50 border border-black/[0.03] dark:border-white/[0.03] shadow-sm">
                                ...
                                <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60">{feat.label}</span>
                              </div>
                            ))}
                          </div>
            ...
            {!isStaff && (
                <motion.div
                  ...
                  transition={{ type: "spring", stiffness: 400, damping: 25 }}
                >
                  <Card className="border-none shadow-2xl bg-card/50 backdrop-blur-xl rounded-[2rem] overflow-hidden group text-left rtl:text-right">
                      ...
                      <CardHeader className="p-8 pb-4">
                          <div className="flex items-center gap-4">
                            ...
                            <div className="space-y-1">
                              ...
                              <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60">{t("dashboard.staff.promos.studentSupport.badge")}</p>
                            </div>
                          </div>
                      </CardHeader>
                      <CardContent className="p-8 pt-2 space-y-6">
                          <p className="text-sm text-muted-foreground font-medium leading-relaxed">
                            {t("dashboard.staff.promos.studentSupport.description").split("AI Study Buddy").map((part, i, arr) => (
                                <React.Fragment key={i}>
                                    {part}
                                    {i < arr.length - 1 && <span className="text-primary font-bold">AI Study Buddy</span>}
                                </React.Fragment>
                            ))}
                          </p>
