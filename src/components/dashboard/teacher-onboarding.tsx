...
import { CheckCircle2, Circle, ArrowRight, Plus, UserPlus, BookOpen, X, Sparkles } from "lucide-react";
            ...
            <CardHeader className="p-8 md:p-12 pb-6 md:pb-8 text-start">
                <div className="flex flex-col md:flex-row md:items-center gap-6">
                    ...
                    <div className="space-y-1.5 flex-1">
                        <CardTitle className="text-2xl md:text-3xl font-black tracking-tight">{t("dashboard.onboarding.teacher.welcome")}</CardTitle>
                ...
                <div className="mt-10 space-y-4">
                    <div className="flex justify-between items-end px-1">
                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">{t("dashboard.onboarding.teacher.setupProgress")}</span>
                ...
                <div className="grid gap-6 md:grid-cols-3">
                    {steps.map((step, index) => (
                        <motion.div
                            ...
                            className={cn(
                                "relative p-6 md:p-8 rounded-[2rem] border transition-all duration-500 flex flex-col group",
                                step.completed 
                        ...
                        >
                            <div className="flex items-start justify-between mb-6">
                                <div className={cn(
                                    ...
                                    step.completed ? "bg-emerald-500/10 text-emerald-600" : "bg-primary/10 text-primary border border-primary/5 group-hover:scale-110"
                                )}>
                            ...
                            <div className="flex-1 space-y-2 text-start">
                                <h3 className={cn(
                                    ...
                                    step.completed ? "text-emerald-600/70" : "text-foreground group-hover:text-primary"
                                )}>{step.title}</h3>
                                <p className="text-sm font-medium text-muted-foreground/70 leading-relaxed">{step.description}</p>
                            </div>
                            {!step.completed && (
                                <Button 
                                    size="lg" 
