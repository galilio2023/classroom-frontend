...
import { useApiUrl, useInvalidate, useGetIdentity, useCustomMutation, useList, HttpError } from "@refinedev/core";
...
export const MagicBuilderDialog = ({ open, onOpenChange, initialClassId }: MagicBuilderDialogProps) => {
  ...
  useEffect(() => {
    ...
    const handleProgress = (data: { step: string; progress: number; classId: number }) => {
      ...
      if (data.progress === 100) {
        setLoading(false);
        setIsCompleted(true);
        toast.success(t("classes.magicBuilder.success" as any, "Curriculum generated successfully!"));
  ...
  }, [open, identity?.id, t, invalidate, classId]);

  const { mutate: startMagicBuilder } = useCustomMutation();

  const handleStart = async () => {
    if (!topic || !subject || !classId) {
      toast.error(t("common.errors.fillRequired" as any, "Please fill all required fields."));
      return;
    }

    setLoading(true);
    setProgress(0);
    setStep(t("common.starting" as any, "Starting AI engine..."));
    ...
    startMagicBuilder(
      ...
      },
      {
        ...
        onError: (error: HttpError) => {
          const message = error.message || t("common.errors.aiFailed" as any, "AI Generation failed.");
          toast.error(message);
          setLoading(false);
        },
      }
          ...
          <div className="p-8 space-y-6">
            ...
            {loading || isCompleted ? (
              <div className="py-10 flex flex-col items-center justify-center space-y-8 animate-in fade-in zoom-in duration-500">
                    ...
                    <div className="space-y-1">
                      ...
                      {!isCompleted && (
                        <p className="text-[9px] font-bold text-muted-foreground animate-pulse">
                          {t("classes.magicBuilder.wait" as any, " Gemini is thinking and structuring...")}
                        </p>
                      )}
                    </div>
                ...
                {isCompleted && (
                  ...
                  >
                    ...
                    {t("buttons.exploreCurriculum" as any, "Explore Curriculum")}
                  </Button>
                )}
              </div>
            ) : (
              <div className="space-y-5">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={classId} onValueChange={setClassId}>
                      <SelectTrigger className="h-14 rounded-2xl border-none bg-muted/50 shadow-inner font-bold">
                        <SelectValue placeholder={isLoadingClasses ? (t("common.loading" as any) as string) : (t("classes.magicBuilder.selectPlaceholder" as any, "Select a class...") as string)} />
                      </SelectTrigger>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground px-1">
                    {t("classes.magicBuilder.subjectLabel" as any, "Subject Area")}
                  </label>
                  <div className="relative group">
                    ...
                    <Input
                      placeholder={t("classes.magicBuilder.subjectPlaceholder" as any, "e.g. Physics, History, Math") as string}
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground px-1">
                    {t("classes.magicBuilder.topicLabel" as any, "Core Topic / Unit Name")}
                  </label>
                  <div className="relative group">
                    ...
                    <Input
                      placeholder={t("classes.magicBuilder.topicPlaceholder" as any, "e.g. Quantum Mechanics, World War II") as string}
                      className="ps-11 h-14 rounded-2xl border-none bg-muted/50 shadow-inner font-bold"
                      value={topic}
