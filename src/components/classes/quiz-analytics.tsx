    ...
    <div className="space-y-6">
                  ...
                  >
                    {stats.map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={
                          entry.correctPercentage < 50
                            ? "hsl(var(--destructive))"
                            : entry.correctPercentage > 80
                              ? "hsl(var(--primary))"
                              : "hsl(var(--primary) / 0.6)"
                        }
                      />
            ...
            {hardestQuestions.length > 0 ? (
              ...
              <div className="py-12 text-center space-y-3 opacity-40">
                ...
                <p className="text-xs font-black uppercase tracking-widest">
                  {t("classes.analytics.perfect", "Class is crushing it!")}
                </p>
              </div>
            )}
          </CardContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <StatItem
          ...
          value={
            hardestQuestions[0]?.correctPercentage
              ? `${hardestQuestions[0].correctPercentage}%`
              : "N/A"
          }
          color="destructive"
...
const StatItem = ({
  ...
  label: string;
  value: string;
  color: "emerald" | "destructive" | "primary";
}) => {
  const bgColorMap = {
  ...
  return (
    <div className="p-6 rounded-4xl bg-card/50 backdrop-blur-xl border border-black/3 dark:border-white/3 flex items-center gap-4">
      <div className={cn("p-3 rounded-2xl", bgColorMap[color])}>{icon}</div>
      <div>
